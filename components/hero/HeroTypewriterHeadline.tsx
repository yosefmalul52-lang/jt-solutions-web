...
export default function HeroTypewriterHeadline({ reduceMotion }: HeroTypewriterHeadlineProps) {
  ...
  const showStatic = !hydrated || reduceMotion === true;

  useEffect(() => {
    if (!hydrated || reduceMotion) {
      setLine1Count(LINE_1.length);
      setLine2Count(LINE_2.length);
      setIsTyping(false);
      return;
    }
  ...
  const visible1 = showStatic ? LINE_1.length : line1Count;
  const visible2 = showStatic ? LINE_2.length : line2Count;
  const showCursor = hydrated && !reduceMotion && isTyping;
    ...
    <h1 className="display-title max-w-4xl mx-auto">
      ...
      <span aria-hidden>
        <span className="block text-white/95">
          <TypewriterChars text={LINE_1} visibleCount={visible1} />
        ...
        <span className="block text-transparent bg-clip-text bg-gradient-to-r from-[#38bdf8] to-[#c084fc]">
          <TypewriterChars text={LINE_2} visibleCount={visible2} />
