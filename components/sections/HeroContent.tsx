...
const HERO_GRID_CLASS =
  "home-hero-scroll-scale relative z-10 mx-auto grid w-full max-w-[90rem] grid-cols-1 items-center gap-10 px-5 py-4 sm:gap-12 sm:px-8 lg:grid-cols-[minmax(0,1fr)_minmax(34rem,40rem)] lg:gap-x-20 lg:gap-y-12 xl:gap-x-24 lg:px-10 xl:px-12";

function HeroScrollShell({
  enabled,
  children,
}: {
  enabled: boolean;
  ...
  return (
    <section
      ref={sectionRef}
      id="hero"
      className="homepage-hero-section relative z-10 flex flex-1 items-center"
      dir="rtl"
    >
      {enabled ? (
        <motion.div
...
export default function HeroContent() {
  ...
  return (
    <HeroScrollShell enabled={useMotion}>
      <Wrap
        className="home-hero-copy flex min-w-0 flex-col text-center lg:max-w-[42rem] lg:justify-self-stretch lg:text-right xl:max-w-[44rem]"
        {...wrapProps}
      >
        ...
        <Text
          className="home-subline home-hero-subline home-hero-copy__subline mx-auto mt-5 max-w-[36ch] md:hidden lg:mx-0 lg:max-w-[42ch]"
          {...itemProps}
        >
          {heroCopy.sublineMobile}
        </Text>
        <Text
          className="home-subline home-hero-subline home-hero-copy__subline mx-auto mt-5 hidden max-w-[42ch] md:block lg:mx-0 lg:max-w-[46ch]"
          {...itemProps}
        >
          {heroCopy.subline}
        </Text>
      ...
      <Item
        className="relative w-full min-w-0 justify-self-center lg:justify-self-end lg:-translate-x-6 xl:-translate-x-12 2xl:-translate-x-16"
        {...itemProps}
      >
        <LeadOrbitVisual />
      </Item>
